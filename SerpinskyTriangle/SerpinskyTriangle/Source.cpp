...
int main()
{
	...
	/*sf::CircleShape triangle(radiusOfTriangle, 3);
	triangle.setPosition(widthScreen / 2 - radiusOfTriangle, heidthScreen / 2 - radiusOfTriangle);
	triangle.setOutlineThickness(thickness);
	triangle.setOutlineColor(sf::Color::White);
	triangle.setFillColor(sf::Color::Black);*/
	std::vector<sf::Vector2f> pointsOfTriangle;
	pointsOfTriangle.push_back({widthScreen/2,50});
	pointsOfTriangle.push_back({ 50, heidthScreen-50});
	pointsOfTriangle.push_back({widthScreen-50, heidthScreen - 50});
	sf::ConvexShape triangle(3);
	triangle.setPoint(0, pointsOfTriangle[1]);
	triangle.setPoint(1, pointsOfTriangle[0]);
	triangle.setPoint(2, pointsOfTriangle[2]);
	/*get_points(triangle, pointsOfTriangle);*/
	triangle.setFillColor(sf::Color::Black);
	triangle.setOutlineThickness(thickness);
	triangle.setOutlineColor(sf::Color::White);
...
sf::Vector2f& gen_first_point_in_triangle(std::vector<sf::Vector2f>& pOt, std::vector<sf::CircleShape>& pointsInTriangle)
{
	...
	while (1)
	{
		...
		if (randX < maxX-thickness || randX > minX+thickness)
		{
			...
			firstPoint.setFillColor(sf::Color::White);
